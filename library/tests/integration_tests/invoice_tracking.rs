    ...
    S: Storage + 'static,
{
    ...
    expected.amount_paid = 1_468_383_460;
    ...
    S: Storage + 'static,
{
    ...
    // Add the invoice.
    let invoice_id = payment_gateway
        .new_invoice(37_419_570, 0, 10, "test invoice".to_string())
        .await
        .expect("failed to add new invoice to payment gateway for tracking");
    let mut subscriber = payment_gateway
        .subscribe(invoice_id)
    ...
    let _txpool_hashes_mock = mock_daemon
        .mock_txpool_hashes("../testing-utils/rpc_resources/txpools/hashes_with_payment.json");
    ...
    S: Storage + 'static,
{
    ...
    // Add transfer to txpool.
    let _txpool_hashes_mock = mock_daemon.mock_txpool_hashes(
        "../testing-utils/rpc_resources/txpools/hashes_with_payment_timelock.json",
    );
    let _transactions_mock = mock_daemon.mock_transactions(
        "../testing-utils/rpc_resources/transactions/hashes_with_payment_timelock.json",
        "../testing-utils/rpc_resources/transactions/txs_with_payment_timelock.json",
    );
    ...
    S: Storage + 'static,
{
    ...
    let output_key = OutputPubKey(
        output
            .get_one_time_key()
            .expect("expected output public key")
            .to_bytes(),
    );
    ...
    let _txpool_hashes_mock = mock_daemon
        .mock_txpool_hashes("../testing-utils/rpc_resources/txpools/hashes_with_payment.json");
    let _transactions_mock = mock_daemon.mock_transactions(
        "../testing-utils/rpc_resources/transactions/hashes_with_payment.json",
        "../testing-utils/rpc_resources/transactions/txs_with_payment.json",
    );
    ...
    S: Storage + 'static,
{
    ...
    // Add the invoice.
    let invoice_id = payment_gateway
        .new_invoice(70_000_000, 2, 7, "invoice 1".to_string())
        .await
        .expect("failed to add new invoice to payment gateway for tracking");
    ...
    // Add the invoice.
    let invoice_id = payment_gateway
        .new_invoice(70_000_000, 2, 7, "invoice 2".to_string())
        .await
        .expect("failed to add new invoice to payment gateway for tracking");
    ...
    // Check that it is as expected.
    expected_1.amount_paid = 37_419_570;
    expected_1.assert_eq(&update);
    ...
    // Check that it is as expected.
    expected_2.amount_paid = 37_419_570;
    expected_2.assert_eq(&update);
    ...
    for height in 2_477_658..2_477_663 {
        ...
        expected_1.expires_in = 2_477_664 - height;
        ...
        expected_2.expires_in = 2_477_664 - height;
    ...
    let txpool_hashes_mock = mock_daemon
        .mock_txpool_hashes("../testing-utils/rpc_resources/txpools/hashes_with_payment_2.json");
    let txpool_transactions_mock = mock_daemon.mock_txpool_transactions(
        "../testing-utils/rpc_resources/transactions/hashes_with_payment_2.json",
        "../testing-utils/rpc_resources/transactions/txs_with_payment_2.json",
    );
    ...
    expected_1.amount_paid = 74_839_140;
    ...
    subscriber_2
        ...
        .await
        .expect_err("should not have received an update, but did");
    let height_mock = mock_daemon.mock_daemon_height(2_477_663);
    ...
    expected_1.current_height = 2_477_663;
    expected_1.assert_eq(&update);
    ...
    expected_2.current_height = 2_477_663;
    expected_2.assert_eq(&update);
    ...
    // Move forward a block.
    let height_mock = mock_daemon.mock_daemon_height(2_477_664);
    ...
    expected_1.is_expired = true;
    expected_1.expires_in = 0;
    expected_1.current_height = 2_477_664;
    expected_1.assert_eq(&update);
    ...
    expected_2.current_height = 2_477_664;
    expected_2.assert_eq(&update);
...
async fn set_initial_height<S>(mut store: S)
where
    S: Storage + 'static,
{
    // Setup.
    ...
    let mock_daemon = MockDaemon::new_mock_daemon().await;
    let _height_mock = mock_daemon.mock_daemon_height(2_477_664);

    // TX from block 2_477_657.
    let tx_hex = hex::decode("").unwrap();
    let tx: monero::Transaction = deserialize(&tx_hex).unwrap();
    let output = &tx.prefix().outputs[1];
    let output_key = OutputPubKey(
    ...
    .daemon_url(mock_daemon.url(""))
    .account_index(1)
    .initial_height(2_477_657)
    .seed(1)
    .rpc_timeout(Duration::from_secs(120))
    .rpc_connection_timeout(Duration::from_secs(60))
    .build()
    .await
    .expect("failed to build payment gateway");
    ...
    // Wait to catch up.
    while payment_gateway_with_height.cache_height()
        < payment_gateway_with_height
            .daemon_height()
            .await
            .unwrap()
    ...
    expected_1.current_height = 2_477_664;
    expected_1.expiration_height = 2_477_671;
    ...
    // Add transaction with same key inserted previously.
    let _txpool_hashes_mock = mock_daemon
        .mock_txpool_hashes("../testing-utils/rpc_resources/txpools/hashes_with_payment.json");
    let _transactions_mock = mock_daemon.mock_transactions(
        "../testing-utils/rpc_resources/transactions/hashes_with_payment.json",
    ...
    // Assert that it is ignored for duplicate key.
    subscriber_1
        .recv_timeout(Duration::from_secs(2))
        .await
        .expect_err("timeout waiting for invoice update");

    // Assert that the other gateway fails to ignore it.
    subscriber_2
        .recv_timeout(Duration::from_secs(120))
        .await
        .expect("timeout waiting for invoice update");
